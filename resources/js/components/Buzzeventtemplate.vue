...
<div v-else>
    ...
    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
        ...
        <input type="search" class="irbu focus:outline-none -ml-8 pl-10 w-full bg-transparent text-base" style="appearance:none;" v-model="search" placeholder="Search by name...">
    </div>
                    ...
                    <div class="w-45">
                        <div class="lg:-mx-10">
                            ...
                            <footer class="relative flex items-center justify-between leading-none p-2 md:p-4 mb-1.5">
                                ...
                                <button @click="showsavelocalstorage(Template.Json)" class="flex no-underline hover: no-underline text-red-dark bg-blue hover:bg-blue-light text-white font-bold text-lg hover:border-blue absolute bg-blue-500 rounded-lg px-1 py-1 transition duration-300 ease-in-out hover:bg-blue-600 right-3" >
                                    <span>Select</span>
                                </button>
                            </footer>
                            </article>
                ...
                <div class="static h-screen w-full absolute flex items-center justify-center bg-modal p-5 " style="overflow: hidden;">
                    <div class="relative bg-white rounded shadow p-8 m-4 max-w-full h-full text-center overflow-y-scroll w-screen" style="height: 100%; overflow-y: hidden;">
                        ...
                        <div style="width: 100% !important; display:flex; justify-content:center;">
                            <button @click="selectedTemplate()" class="flex no-underline hover: no-underline text-red-dark bg-blue hover:bg-blue-light text-white font-bold text-lg hover:border-blue absolute bg-blue-500 rounded-lg px-1 py-1 transition duration-300 ease-in-out hover:bg-blue-600 bottom-2 w-25 align-center justify-center" >
            ...
            <div class="modal-overlay " v-if="showModalS" >
                <div class="static h-screen w-full absolute flex items-center justify-center bg-modal p-5 " style="overflow: hidden;">
                    <div class="relative bg-white rounded shadow p-8 m-4 max-w-full h-full text-center overflow-y-scroll w-screen" style="height: 100%; overflow-y: hidden;">
                        ...
                        <select v-model="valTop" @change="allchange()" class="border border-gray-300 rounded-full text-gray-600 h-10 pl-5 pr-10 bg-white hover:border-gray-400 focus:outline-none appearance-none">
                            <option disabled selected>Apply one design to all events</option>
                            <option>Horizontal</option>
                            <option>Vertical</option>  
                        </select>
                        <div v-if="EventsisFetched" class="flex flex-wrap bg-white justify-center w-full h-full sm:rounded-lg bg-white overflow-hidden sm:rounded-lg shadow-2xl mb-3">
                            ...
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                                ...
                                <input type="search" class="irbu -ml-8 pl-10 max-h-10 focus:outline-none w-full bg-transparent text-base" style="appearance:none;" v-model="searchevent" placeholder="Search by name ...">
                            </div>
                                    ...
                                    <div class="sm:px-20 bg-white max-w-20xl mx-10 sm:px-5 lg:px-5">
                                        <div class="my-10 ml-2">
                                            <div class="w-45">
                                                <div class="lg:-mx-10">
                                                    <article class="float-none overflow-hidden w-80 rounded-lg shadow-lg">
                                                        <img alt="Template_image" class="block w-full" style="height:9rem !important" :src="Event.logo">
                                                    <header class="flex justify-between leading-tight p-2 md:p-4 justify-center text-c">
                                                        <h1 class="text-lg">
                                                                {{Event.name}}
                                                        </h1>
                                                    ...
                                                    <div v-if="frvalue">
                                                        <select id="mySelect" v-model="Event.id">
                                                    ...
                                                    <footer class="relative flex items-center justify-between leading-none p-2 md:p-4 mb-4">
                                                        <vb-switch type="" size="large" v-model="value" @change="change(Event.id,Event.name,Event.color,Event.pic,Event.date_debut,Event.date_fin)"/>                                                 
                                                    </footer>
                                                    </article>
                                                </div>
                                            </div>
                        ...
                        <div style="width: 100% !important; display:flex; justify-content:center;">
                            <button @click="gotoRoute()" class="flex no-underline hover: no-underline text-red-dark bg-blue hover:bg-blue-light text-white font-bold text-lg hover:border-blue absolute bg-blue-500 rounded-lg px-1 py-1 transition duration-300 ease-in-out hover:bg-blue-600 bottom-2 w-25 align-center justify-center" >
...
<script>
    ...
    export default {
        ...
        methods:{
            allchange() {
                console.log(this.valTop)
                if(this.valTop=='Horizontal'){
                this.frvalue=false
                console.log('All Horizontal');
                this.allselected= 'H'
                }
                else if(this.valTop=='Vertical'){
                    this.frvalue=false
            ...
            change (id,name,color,pic,dated,datef) {
                if(this.allselected!= null ){
                    if(this.allselected=='H' && this.value==true){
                        ...
                        console.log(this.selecteddesign)
                    }else if(this.allselected !=null && this.value==false){
        ...
        async mounted() {
           await axios.get('/getBuzzeventsTemplates')
                .then((response) =>{
        ...
        filteredEvents() {
          return this.Events.filter(Event => {
            return Event.name.toLowerCase().includes(this.searchevent.toLowerCase())
          })
        }
      }
